...
def draw_latent_from_emb(png_fname, z_np, color_list, args):
    ...
    if args.rep_umap:  # 再現性のあるumapを作る
        umap_model = umap.UMAP(
            n_neighbors=args.nei, random_state=42
        )  # n_neighborsのデフォルトは15
    else:  # 再現性のないumapを作る
    ...
    if args.points:
        ...
        for p_inf in points:
            ...
            rect = Rectangle(
                (left), width, hight, linewidth=2, edgecolor="red", facecolor="none"
            )
            ax.add_patch(rect)
            ax.text(
                left[0],
...
def readPoints(fname: str):
    ...
    with fh as f:
        for line in f:
            ...
            if len(parts) < 3:
                continue
            p1, p2, name = parts[0], parts[1], parts[2]
            l.append((p1, p2, name))
    return l
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rep_umap", action="store_true", help="make a non-reproducible UMAP"
    )  #
    ...
    parser.add_argument(
        "--no_color_scale", action="store_true", help="do not show the color scale"
    )
