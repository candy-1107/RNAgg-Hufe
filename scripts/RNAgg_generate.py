...
# Ensure project scripts directory is available on sys.path, fallback to ~/pyscript
...
def main(args: dict):
    ...
    d_rep, max_len, model_type, nuc_only = (
        checkpoint["d_rep"],
        checkpoint["max_len"],
        checkpoint["type"],
        checkpoint["nuc_only"],
    ...
    if model_type == "act":
        if nuc_only == "yes":
            model = RNAgg_VAE.MLP_VAE_REGRE(
                max_len * (word_size), max_len * (word_size), d_rep, device=device
            ).to(device)
        else:
            model = RNAgg_VAE.MLP_VAE_REGRE(
                max_len * (word_size + G_DIM),
                max_len * (word_size + G_DIM),
                d_rep,
                device=device,
            ).to(device)
    ...
    if nuc_only == "yes":
        ...
        yy = y.view(
            s_z[0], max_len, word_size + G_DIM
        )  # yyはlogitなので、0-1に正規化する必要がある。
    ...
    if nuc_only == "yes":
        ...
        for i in range(0, s[0], args.s_bat):
            ...
            yy_batch = yy[
                i : i + args.s_bat
            ]  # スライスの終わりがyyのサイズを超えても、yyの最大サイズになる。
    ...
    # 最終出力
    fout = utils.open_text(args.outfile, "w")
...
def generateOneSeq(
    n, pssm_batch_log, StoS, StoT, TtoT, TtoU, TtoTU, pair_numera, idx2token
):
    ...
    # print(TR)
    TB = [
        [[(np.nan, np.nan, np.nan) for j in range(s[1])] for i in range(s[1])]
        for st in range(3)
    ]  # 3 x L x Lの行列（Pathを記録する）
    ...
    for l in range(2, MIN_LOOP + 2):  # 長さ2から４まで
        for i in range(s[1] - l + 1):
            j = i + l - 1
            S[i, j] = (
                max(pssm_log[i, :6]) + StoS[n, i, j] + S[i + 1, j]
            )  # StoSが文法の部分、maxが塩基の部分
    ...
    for l in range(MIN_LOOP + 2, s[1] + 1):  # 長さ5からLまで
        for i in range(s[1] - l + 1):
            ...
            if l >= 10:  # 長さ10以上の時のみBifucationを考えることとする。
                ...
                if T[i, j] < TtoTU[n, i, j] and TtoTU[n, i, j] > np.log(
                    1e-5
                ):  # ある程度確率が高い場合しか考えない。
                    ...
                    for k in k_set:
                        if k < i + 1 or j - 1 < k:
                            continue
                        score = (
                            TtoTU[n, i, j] + pssm_log[k, 16] + T[i, k] + U[k + 1, j]
                        )  # S->TU遷移の確率
                        if T[i][j] < score:
                            # bif = 1
                            T[i][j] = score
                            TR[I["T"]][i][j] = [
                                (I["T"], i, k),
                                (I["U"], k + 1, j),
                            ]  # listかtupleかでbifucationかどうかを区別することにする。
                            TB[I["T"]][i][j] = ""
                        # exit(0)
            # state S
            ## rule 1: S->aS
            ...
            S[i, j] = (
                max(pssm_log[i, :5]) + StoS[n, i, j] + S[i + 1, j]
            )  # Sからxは出力されない
...
def generateRNA(pssm_batch, idx2token, args):
    ...
    # A:0, C:1, G:2, U:3
    mAU = pssm_batch_log[:, :, 0].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 3].view(
        s[0], s[1], 1
    )  # AU
    mUA = pssm_batch_log[:, :, 3].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 0].view(
        s[0], s[1], 1
    )  # UA
    mGC = pssm_batch_log[:, :, 2].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 1].view(
        s[0], s[1], 1
    )  # GC
    mCG = pssm_batch_log[:, :, 1].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 2].view(
        s[0], s[1], 1
    )  # CG
    mGU = pssm_batch_log[:, :, 2].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 3].view(
        s[0], s[1], 1
    )  # GU
    mUG = pssm_batch_log[:, :, 3].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 2].view(
        s[0], s[1], 1
    )  # UG
    ...
    # 全i,jに対するS->aS遷移
    StoS = pssm_batch_log[:, :, 6].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 7].view(
        s[0], s[1], 1
    )  # StoT
    StoS = torch.transpose(StoS, 1, 2)

    # ブロードキャストを利用して、全i,jに対するS->T遷移と、T->U遷移のスコアを求める。
    StoT = pssm_batch_log[:, :, 8].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 9].view(
        s[0], s[1], 1
    )  # StoT
    StoT = torch.transpose(StoT, 1, 2)

    # 全i,jに対するT->Ta遷移
    TtoT = pssm_batch_log[:, :, 10].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 11].view(
        s[0], s[1], 1
    )  # StoT
    TtoT = torch.transpose(TtoT, 1, 2)

    TtoU = pssm_batch_log[:, :, 12].view(s[0], 1, s[1]) + pssm_batch_log[:, :, 13].view(
        s[0], s[1], 1
    )  # TtoU
    TtoU = torch.transpose(TtoU, 1, 2)

    # ブロードキャストを利用して、全i,jに対するT->TUの遷移スコアを求める。
    TtoTU = pssm_batch_log[:, :, 14].view(s[0], 1, s[1]) + pssm_batch_log[
        :, :, 15
    ].view(
        s[0], s[1], 1
    )  # TtoTU
    TtoTU = torch.transpose(
        TtoTU, 1, 2
    )  # tranposeしなくて良いかも。broad castを変えれば良さそう。

    # 次元を分けたことにより、transposeが必要になった。

    pair_numera = {
        1: mAU.to("cpu").detach().numpy().copy(),
...
def getBP(ss: str):
    ...
    for i in range(len(ss)):
        ...
        elif ss[i] == "." or ss[i] == "," or ss[i] == ":" or ss[i] == "_":
            pass
        else:
...
def TraceBack(st_ini, pos_ini, TR, TB, seq, ss):
    ...
    while len(stack) > 0:
        ...
        if type(next) is list:  # リスト型であれば、bifucation
            ...
            if np.isnan(next[0]):
                print(
                    f"Traceback error: you accessed undefined element at {i,j} in TR matrix",
                    file=sys.stderr,
                )
                exit(0)
            ...
            print(
                f"Unknown type of class obtained by traceback {next}", file=sys.stderr
            )
            exit(0)
...
def readInput(fname: str):
    ...
    with fh as f:
        for line in f:
            ...
            if len(parts) < 3:
                continue
            sid, seq, ss = parts[0], parts[1], parts[2]
            d_seq[sid] = seq
            d_ss[sid] = ss
    return d_seq, d_ss
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--outProb", action="store_true", help="output TtoU probability matrix"
    )
    ...
    parser.add_argument(
        "--from_emb", action="store_true", help="input embedding directly"
    )
