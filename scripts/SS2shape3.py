...
def generate_rule_G4b(ID, r, seq, ss, bp, pos, state):  # STはステートのこと
    ...
    if state == "S":
        ...
        if ss[i] == ".":  # 左側が'.'
            ...
            print(
                f"Unknown error at {i,j}, state={state}, ss[i] and ss[j]={ss[i], ss[j]}",
                file=sys.stderr,
            )
...
def generate_rule_G4b_org(ID, r, seq, ss, bp, pos, st):  # STはステートのこと
    ...
    if st == "S":
        ...
        if ss[i] == ".":  # 左側が'.'
            ...
            print(
                f"Unknown error at {i,j}, state={st}, ss[i] and ss[j]={ss[i], ss[j]}",
                file=sys.stderr,
            )
            print(seq, file=sys.stderr)
            print(ss, file=sys.stderr)
...
def makeGraph(
    rules: list,
):  # これは古いルールのフォーマットに基づいており、使うためには改良が必要(2024/11/20)
    ...
    while len(stack) > 0:
        node_id = stack.pop()
        node_type = rules[node_id][
            0
        ]  # rulesの最初の要素にはルールのタイプがある(e, p, b)のいずれか
