...
def main(args: dict):
    ...
    d_rep, max_len, model_type, nuc_yes_no = (
        checkpoint["d_rep"],
        checkpoint["max_len"],
        checkpoint["type"],
        checkpoint["nuc_only"],
    ...
    sid2act = dict(
        [(sid, np.nan) for sid in sid_list]
    )  # utils.Datasetを使うためのダミーデータ
    act_list = [sid2act[sid] for sid in sid_list]
    B_mat = Binary_matrix.makeMatrix(
        sid2seq, sid2ss, sid_list, word_size, G_DIM, token2idx, nuc_yes_no
    )
