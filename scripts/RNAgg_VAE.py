...
class ConvDecoder(nn.Module):
    def __init__(self, start_in_ch, n_layers=3, input_hw=(8, 8)):
        super().__init__()
        layers = []
        in_ch = start_in_ch
        H, W = input_hw
...
class Conv_VAE(nn.Module):
    def __init__(self, input_shape, latent_dim=128, n_layers=3, reduction=3, device='cpu'):
        ...
        self.decoder = ConvDecoder(start_in_ch=enc_ch, n_layers=n_layers, input_hw=(enc_h, enc_w))
        dec_out_ch = enc_ch // (2 ** n_layers)
        self.decoder.attention = ChannelSELayer(dec_out_ch, reduction=reduction)
    ...
    def forward(self, x):
        ...
        c, h1_d, w1_d = self.enc_shape
        h_conv = h_flat_decode.view(b, c, h1_d, w1_d)
        x_rec = torch.sigmoid(self.decoder(h_conv))
        return x_rec, mu, var
